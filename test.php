...
<html><body><center><canvas id="c" width=400 height=400></canvas><div id="x" style="width:400px; font-size:200%; text-align:center"></div></center><script>
...
function U(a) {
    var b = a.length - 1;
    H.beginPath();
    for (H.moveTo(xs[a[b]], ys[a[b]]); b--; )
        H.lineTo(xs[a[b]], ys[a[b]]);
    H.closePath()
}
function tf(a, b, i, f) {
    a = [[Q[a][0], Q[a][1], Q[a][2]], [Q[a][2], Q[a][3], Q[a][0]]];
    for (var d = 0; d < 2; d++) {
        var e = a[d];
        H.save();
        U(e);
...
, function(a) {
    var b, i = [[Q[a][0], Q[a][1], Q[a][2]], [Q[a][2], Q[a][3], Q[a][0]]];
    a = (a ^ a >> 4) & 2 ? 1 : 7;
    for (var f = 0; f < 2; f++) {
        var d = i[f];
        U(d);
        ...
        if (e == h)
            H.fillStyle = "rgb(" + (a & 1) * e * 4 + "," + (a & 2) * e * 2 + "," + (a & 4) * e + ")";
        else {
            b = xs[d[0]];
            var j = ys[d[0]]
              , k = xs[d[2]]
